function AppRun($rootScope, $state, _) {
  ...
  $rootScope.$on('$stateChangeStart', function(event, toState, toParams, fromState, fromParams, options) {
    if (routingHelper.isCurrentState(toState, toParams) && !options.reload) {
      ...
      // that was casuing that can cause multple transition events to fire for
      // the same route when a  ``connect`` url parameter is present
